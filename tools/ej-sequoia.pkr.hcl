...
variable "mdm_challenge" {
  type        = string
  description = "MDM enrollment challenge string"
  default     = "114462661045031677246004832591719061792"
}
...
build {
  ...
  provisioner "shell" {
    inline = [
      "cat << EOF > ~/Desktop/mdm_enroll.webloc",
      "<?xml version=\"1.0\" encoding=\"UTF-8\"?>",
      "<plist version=\"1.0\">",
      "<dict>",
      "    <key>URL</key>",
      "    <string>${var.jamf_url}/enroll?invitation=${var.mdm_challenge}</string>",
      "</dict>",
      "</plist>",
      "EOF",
    ]
