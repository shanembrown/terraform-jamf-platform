...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.800-171.audit').objectForKey('os_image_generation_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.800-171.audit').objectForKey('os_image_generation_disable'))["exempt_reason"]
EOS
)   
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: 800-171 - os_image_generation_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: 800-171 - os_image_generation_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: 800-171 - os_image_generation_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    # expected result {'string': 'Sequoia'}
...
EOS
    ...
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "Sequoia" ]]; then
        logmessage "os_screensaver_loginwindow_enforce passed (Result: $result_value, Expected: \"{'string': 'Sequoia'}\")"
        ...
        /usr/bin/logger "mSCP: 800-171 - os_screensaver_loginwindow_enforce passed (Result: $result_value, Expected: "{'string': 'Sequoia'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "os_screensaver_loginwindow_enforce failed (Result: $result_value, Expected: \"{'string': 'Sequoia'}\")"
            /usr/bin/defaults write "$audit_plist" os_screensaver_loginwindow_enforce -dict-add finding -bool YES
            if [[ ! "$customref" == "os_screensaver_loginwindow_enforce" ]]; then
                /usr/bin/defaults write "$audit_plist" os_screensaver_loginwindow_enforce -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: 800-171 - os_screensaver_loginwindow_enforce failed (Result: $result_value, Expected: "{'string': 'Sequoia'}")"
        else
            logmessage "os_screensaver_loginwindow_enforce failed (Result: $result_value, Expected: \"{'string': 'Sequoia'}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" os_screensaver_loginwindow_enforce -dict-add finding -bool YES
            if [[ ! "$customref" == "os_screensaver_loginwindow_enforce" ]]; then
              /usr/bin/defaults write "$audit_plist" os_screensaver_loginwindow_enforce -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: 800-171 - os_screensaver_loginwindow_enforce failed (Result: $result_value, Expected: "{'string': 'Sequoia'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
