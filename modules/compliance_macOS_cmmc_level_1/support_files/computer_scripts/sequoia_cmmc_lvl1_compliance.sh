...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cmmc_lvl1.audit').objectForKey('os_image_generation_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cmmc_lvl1.audit').objectForKey('os_image_generation_disable'))["exempt_reason"]
EOS
)   
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: cmmc_lvl1 - os_image_generation_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cmmc_lvl1 - os_image_generation_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: cmmc_lvl1 - os_image_generation_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
